package com.js.phonicdiary.view.passcodeview;
...
import com.js.phonicdiary.fingerprint.FingerprintCallback;
import com.js.phonicdiary.fingerprint.FingerprintVerifyManager;
...
public class PasscodeView extends FrameLayout implements View.OnClickListener {
    ...
    public PasscodeView(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        correctInputTip = correctInputTip == null ? getContext().getString(R.string.Passcode_is_correct) : correctInputTip;
...
//        tintImageView(iv_lock, numberTextColor);
    ...
    public void useFingerprint(){
        if (passcodeType == PasscodeView.TYPE_CHECK_PASSCODE) {
            if (LogE.isLog) {
                LogE.e("wbb","isOpenFingerprintCodedLock: "+AppPreferences.isOpenFingerprintCodedLock());
            }
            if (AppPreferences.isOpenFingerprintCodedLock()) {
                mFingerprint.setVisibility(VISIBLE);
                fingerprintVerifyManager = new FingerprintVerifyManager();
                fingerprintVerifyManager.startVerify(new FingerprintCallback() {
                    @Override
                    ...
                    public void onSucceeded() {
                        if (listener != null) {
                            listener.onSuccess(AppPreferences.getLocalPasscode());
                        }
                    }
    ...
    public void cancel() {
        if (fingerprintVerifyManager != null) {
            fingerprintVerifyManager.cancel();
    ...
    public PasscodeView setPasscodeType(int passcodeType) {
...
//                .start();
    ...
    public static int TYPE_SET_PASSCODE = 0;
    public static int TYPE_CHECK_PASSCODE = 1;
